...
<p class="bottom-three">Answer: 756 translates to 111 101 110, meaning the full (read, write, and execute) permissions for user, read and execute permissions for group, and read and write permissions for all other people.</p>
3. Write a command to give `rw-rwx--x` permissions to file "foo".
<p class="bottom-three">Answer: the given permissions translate to 671. This can be done using `chmod 671 foo`. </p>
...
<p class="bottom-three">Answer: Use `export` command to make it an environment variable.</p>
...
<p class="bottom-three">Answer: `find ~ -name "*[5-7]*"`</p>
...
<p class="bottom-three">Answer: press Ctrl+Z keys.</p>
...
<p class="bottom-three">Answer: ```ps aux | wc -l```</p>

GCC, Makefile and GDB
---

1. In the compilation process, describe the job of a linker?
<p class="bottom-three">Answer: it merges the object files into a single executable file.</p>
1. What is the difference between `next` and `continue` commands in gdb?
<p class="bottom-three">Answer: `next` goes to the next line but `continue` goes to the next breakpoint.</p>
1. Write the following compilation stages in correct order: linking, preprocessing, assembling, compilation
<p class="bottom-three">Answer: preprocessing->compilation->assembling->linking</p>
...
<p class="bottom-three">Answer: `bt` or `backtrace`</p>
...
<p class="bottom-three">Answer: This may cause segmentation fault. Because the program access to low memory address `10` which is (usually in kernel space) and is inaccessible to user-space program.</p>
